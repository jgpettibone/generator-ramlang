...
var getParametersForMethod = function(method, relativeUri) {
  ...
  // UPDATE: allows for both {id} or {resourceId}
  var hasId = relativeUri.match(/\{[a-z]*[iI]d}/);
  var uri = relativeUri.replace(/\/{[a-z]*[iI]d}$/i, '');
  ...
  segments.forEach(function(segment, index) {
    ...
    if ((/{[a-z]*[iI]d}/).test(segment)) {
      ...
      // If the segment is not an id parameter then just add it to the uri segments
      ...
      // Only add an id parameter if the uri has an id and if it's not the
      // last resource in the uri
      if (hasId && index < segments.length - 2) {
  ...
  // UPDATE: Defaults to 'id' but allows 'resourceId'
  var currentId = 'id';
  if (hasId) {
    var currentId = hasId[0].replace(/[{}]/g, '');
  }
  ...
  switch (method) {
    ...
    case 'put':
      params.push('entity');
      apiParams.push('entity.' + currentId, 'entity');
      break;
    case 'delete':
      params.push(currentId);
      apiParams.push(currentId);
...
var isCollection = function(ramlResource) {
  ...
  if (ramlResource.type) {
    // Iterate through the type object and check if any of the keys contains
    // the word collection.
    for (var key in ramlResource.type) {
      if (key && key.toLowerCase && key.toLowerCase().indexOf('collection') > 0) {
        return true;
      }
    }
  }

  if (!ramlResource.type || !ramlResource.type.toLowerCase) {
    return false;
  }
...
var generateSubResource = function(ramlResource, relativeUri) {
  ...
  // Simply flatten out the sub-resources. This is not necessarily the best
  // way but it is at least consistent with the way the corresponding server
  // side codegen works at https://github.com/mulesoft/raml-jaxrs-codegen
  // where it also flattens out the structure.
  //
  // "/sweet-fruit"                    ->  "SweetFruit"
  // "/sweet-fruit/banana"             ->  "SweetFruitBanana"
  // "/sweet-fruit/banana/yellow-peel" ->  "SweetFruitBananaYellowPeel"
  //
  var subResourceName = generatorUtil.toCamelCase(
      ramlResource.relativeUri.replace('/', '').replace(/\//g, '_'), true);
...
var recursResources = function(ramlResource, level, relativeUri) {
  ...
  if (level != 0) {
    // UPDATE: Allows for both {id} and {resourceId}
    if (isCollection(ramlResource) || !(/{[a-z]*[iI]d}/).test(ramlResource.relativeUri)) {
  ...
  if (ramlResource.resources && ramlResource.resources.length > 0) {
    ramlResource.resources.forEach(function(resource) {
      // UPDATE: Allows for both {id} and {resourceId}
      var isCol = isCollection(resource) || !(/{[a-z]*[iI]d}/).test(resource.relativeUri);
 ...
 * @returns {string} The compiled template string
 */
module.exports.generate = function(ramlResource, _providerName_) {
