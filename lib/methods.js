...
var getParametersForMethod = function(ramlResource, method, relativeUri) {
  ...
  // UPDATE: Allow for both {id} and {<resource>Id}
  ...
  segments.forEach(function(segment, index) {
    ...
    _.forEach(uriParams, function(paramValue, paramName) {
      ...
      if (paramValue.required === true || paramValue.required == 'true') {
        ...
        // UPDATE: Allow for both {id} and {<resource>Id}
        else if (!/[a-z]*id$/ig.test(paramName.toLowerCase())) {
          // console.log('   inside the !=id else if')
    ...
    if (startsWithIdParam(segment)) {
      ...
      // console.log('starts with id param');
  ...
  // console.log('manuallyResolve', manuallyResolve);
  ...
  console.log('apiParams', apiParams, '/n/n');
...
var isCollection = function(ramlResource) {
  ...
  console.log('LOOK HERE ramlResource: ', ramlResource);
  console.log('ramlResource.uriParameters: ', ramlResource.uriParameters);
  console.log('ramlResource.relativeUri: ', ramlResource.relativeUri);
  console.log('test empty ramlresource', !_.isEmpty(ramlResource));
  console.log('_.isEmpty(ramlResource.uriParameters)', _.isEmpty(ramlResource.uriParameters));
  console.log('!/^\/{[a-z]*id}/ig.test(ramlResource.relativeUri)',!/^\/{[a-z]*id}/ig.test(ramlResource.relativeUri));
...
var generateSubResource = function(ramlResource, relativeUri) {
  ...
  // console.log('subMethodName', subMethodName);
