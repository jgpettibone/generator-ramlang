...
var getParametersForMethod = function(ramlResource, method, relativeUri) {
  ...
  console.log('all matches', /{[a-z]*[iI]d}/g.match(relativeUri));
  console.log('lastId', relativeUri.match(/\{[a-z]*[iI]d}$/));
  console.log('method', method);

  if (hasId && hasId[0] !== '{id}') {
    currentId = hasId[0].replace(/[{}]/g, '');
  }
...
var isCollection = function(ramlResource) {
  ...
  var hasId = /^\/{[a-z]*id}/ig.test(ramlResource.relativeUri);
  var currentId = 'id';
  if (hasId) {
    currentId = ramlResource.relativeUri.replace(/[\/{}]/g, '');
  }
