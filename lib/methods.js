...
var getParametersForMethod = function(ramlResource, method, relativeUri) {
  ...
  segments.forEach(function(segment, index) {
    ...
    _.forEach(uriParams, function(paramValue, paramName) {
      if (paramValue.required === true || paramValue.required == 'true') {
        ...
        // UPDATE: Allow for both 'id' and '<resource>Id'
        // Assumes no other resources ending in 'id' or 'Id'
  ...
  // UPDATE: Allow for both {id} and {<resource>Id}
  var currentId = 'id';
  // var hasId = relativeUri.match(/\{[a-z]*id\}/);
  // if (hasId) {
  //   currentId = hasId[0].replace(/[{}]/g, '');
  // }
  var allIds = relativeUri.match(/\{[a-z]*id\}/ig);
  if (allIds) {
...
var isCollection = function(ramlResource) {
  ...
  // The input format would be either '/{id}' or '/{<resource>Id}'
