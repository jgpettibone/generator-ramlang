...
var getParametersForMethod = function(ramlResource, method, relativeUri) {
  ...
  apiParams.push(apiUri);
   ...
   // UPDATE: Allows 'id' or '<resource>Id'
  var currentId = 'id';
  var allIds = relativeUri.match(/\{[a-z]*[iI]d}/g);
  if (allIds) {
    // Uses the last id in the relativeUri since that is the relevant one
    currentId = allIds[allIds.length - 1].replace(/[{}]/g, '');
  }
...
var isCollection = function(ramlResource) {
  ...
  // Check for both id and <resource>Id in relativeUri and key in uriParameters
  var isId = /^\/{[a-z]*id}/ig.test(ramlResource.relativeUri);
  currentId = 'id';
  if (isId) {
    var currentId = ramlResource.relativeUri.replace(/[\/{}]/g, '');
  }
  return (!_.isEmpty(ramlResource) &&
             (_.isEmpty(ramlResource.uriParameters) ||
              _.isUndefined(ramlResource.uriParameters[currentId]))) ||
          !isId;
};
